...
class CartController extends GetxController {
  ...
  Future<void> fetchCartItems() async {
    final response = await _supabaseClient.from('Users').select().eq("Uid", _supabaseClient.auth.currentUser!.id);
    cartIdList = response[0]['cartList'];
    for (int i = 0; i < cartIdList.length; i++) {
      final cartResponse = await _supabaseClient.from('Cart_Items').select().eq("cart_id", cartIdList[i]);
      final productResponse = await _supabaseClient.from('Products').select().eq("product_id", cartResponse[0]['product_id']);
      cartList.add(CartItem(cartIdList[i], cartResponse[0]['quantity'], cartResponse[0]['color'], productResponse[0]));
      total.value = total.value + (cartList.elementAt(i).quantity * cartList.elementAt(i).price);
    }
  }
  ...
  Future<void> addToCart(Product product, Color color, {int quantity = 1, bool showSnackbar = true}) async {
    ...
    if (index != -1) {
      ...
      await _supabaseClient.from('Users').update({'cartList': cartIdList}).eq("Uid", _supabaseClient.auth.currentUser!.id);
    }
    if (showSnackbar) {
      Get.snackbar(
        "Added to Cart",
