...
class AddPaymentController extends GetxController {
  ...
  Future<void> addCardDetail() async {
    final insertData = await _supabaseClient
        .from("Card_Details")
        .insert({"cardholder_name": name.value, "card_number": cardNumber, "month": month, "year": year, "user_id": _supabaseClient.auth.currentUser!.id}).select();
    if (_cardDetailsController.cardDetailList.isEmpty) {
      ...
      await _supabaseClient.from("Users").update({'default_card_detail_id': insertData[0]['id']}).eq(
        "Uid",
